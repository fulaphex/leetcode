...
impl Solution {
    pub fn generate(num_rows: i32) -> Vec<Vec<i32>> {
        ...
        return res[..num_rows as usize].to_vec();
        */
        let mut res = vec![vec![1]];
        for idx in 0..num_rows - 1 {
            let prev_row = &res[res.len() - 1];
            let row: Vec<i32> = iter::zip(
                res[res.len() - 1].iter().chain([&0]),
                [0].iter().chain(res[res.len() - 1].iter()),
            )
            .map(|(x, y)| x + y)
            .collect();
            res.push(row);
...
mod tests {
    ...
    fn test1() {
        ...
        assert_eq!(Solution::generate(num_rows), res);
    }
